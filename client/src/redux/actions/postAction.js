...
export const createPost = ({content, images, auth, socket}) => async dispatch => {
    ...
    try {
        ...
        dispatch({
            ...
            payload: {
                error:  err.response && err.response.data ? err.response.data.msg : 'Unknown error',
            }
        })
    }
}
...
const sendPdf = async (pdfFile) => {
  const formData = new FormData();
  formData.append('pdfFile', pdfFile);

  try {
    const res = await axios.post('/api/pdf/sendpdf', formData, {
      headers: {
